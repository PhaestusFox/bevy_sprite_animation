...
#[cfg(not(feature = "serialize"))]
impl<T> MatchType for T where T: 'static + Send + Sync + Eq + std::hash::Hash + Debug {}
...
where
    T: MatchType + serde::de::DeserializeOwned + serde::Serialize + std::any::Any + TypePath,
{
...
impl<T: MatchType + serde::de::DeserializeOwned + TypePath> LoadNode for MatchNode<T> {
    fn load<'b>(
        ...
        let check = map.get("check").ok_or(LoadError::Ron(SpannedError {
            code: ron::Error::MissingStructField {
                field: "Check",
                outer: Some(std::any::type_name::<Self>().to_string()),
            },
            position: chars.file_position(),
        }))?;
        let pairs = map.get("pairs").ok_or(LoadError::Ron(SpannedError {
            code: ron::Error::MissingStructField {
                field: "Pairs",
                outer: Some(std::any::type_name::<Self>().to_string()),
            },
        ...
        let default = map.get("default").ok_or(LoadError::Ron(SpannedError {
            code: ron::Error::MissingStructField {
                field: "Default",
                outer: Some(std::any::type_name::<Self>().to_string()),
            },
        ...
        let name = map.get("name").ok_or(LoadError::Ron(SpannedError {
            code: ron::Error::MissingStructField {
                field: "Name",
                outer: Some(std::any::type_name::<Self>().to_string()),
            },
        ...
        let check: Attribute = ron::from_str(check)
            .or_else(|e| Err(LoadError::Ron(e).add_offset(chars.file_position())))?;
        let pairs: Vec<(T, NodeId)> = ron::from_str(pairs)
            .or_else(|e| Err(LoadError::Ron(e).add_offset(chars.file_position())))?;
